...
3. The public Judge Mode shows `LIVE / NEBIUS`, the exact NVIDIA model identifier, a redacted
   request ID, the trace ID and the final Digital Twin mission result.
4. `DIGITAL TWIN / SIMULATION` and `NO PHYSICAL CLAIM` remain visible on simulator frames.

## Required recapture

Replace all three files in one capture session:

- `01-completed.png`: completed Digital Twin mission, final pose and simulation outcome.
